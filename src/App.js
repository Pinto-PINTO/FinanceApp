...
export default function FinanceTrackerApp() {
  ...
  const handleAddTransaction = () => {
    ...
    if (editingTransaction) {
      ...
      setTransactions([
        { ...formData, id: Date.now(), amount: parseFloat(formData.amount) },
        ...transactions,
      ]);
    }
  ...
  const handleAddCategory = () => {
    ...
    if (editingCategory) {
      ...
      setCategories([
        ...categories,
        { id: Date.now(), ...categoryFormData, parentId: null },
      ]);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">
          Budget Overview (Click to view details)
        </h3>
        <div className="space-y-4">
          {stats.categorySpending.slice(0, 5).map((cat) => (
            <div
              key={cat.id}
  ...
  const CategoryDetailsScreen = () => {
    ...
    const subTransactions = transactions.filter((t) =>
      subcats.find((sc) => sc.id === t.category)
    );
    const allTransactions = [...catTransactions, ...subTransactions];
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <div className="text-2xl font-bold text-green-600">
                ${Math.max(0, cat?.budget - totalSpent).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
            <div
              ...
              style={{
                width: `${Math.min(
                  (totalSpent / cat?.budget) * 100 || 0,
                  100
                )}%`,
                backgroundColor:
                  totalSpent / cat?.budget > 1 ? "#EF4444" : cat?.color,
              }}
            />
          </div>
          <div className="text-sm text-gray-600">
            {((totalSpent / cat?.budget) * 100).toFixed(0)}% used
          </div>
        </div>

        {subcats.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold mb-4">Subcategories</h3>
            <div className="space-y-3">
              {subcats.map((subcat) => {
                const subSpent = transactions
                  .filter((t) => t.category === subcat.id)
                  .reduce((sum, t) => sum + t.amount, 0);
                const subPercent =
                  subcat.budget > 0 ? (subSpent / subcat.budget) * 100 : 0;
                return (
                  <div
                    key={subcat.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() =>
                            setCategories(
                              categories.filter((c) => c.id !== subcat.id)
                            )
                          }
                    ...
                    <div className="text-xs text-gray-500 mb-2">
                      ${subSpent.toFixed(2)} / ${subcat.budget.toFixed(2)} (
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor:
                            subPercent > 100 ? "#EF4444" : subcat.color,
                        }}
                      />
                    </div>
                  </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">{trans.date}</div>
                  {trans.note && (
                    <div className="text-xs text-gray-400">{trans.note}</div>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3">
        ...
        {accounts.map((acc) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div className="text-lg font-bold" style={{ color: acc.color }}>
  ...
  const BudgetScreen = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <h2 className="text-xl font-bold mb-6">
        Budget Tracker (Click to view transactions)
      </h2>
      <div className="space-y-6">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-5 px-4 py-3">
          <button
            ...
            className={`flex flex-col items-center gap-1 ${
              currentScreen === "home" && !selectedCategoryId
                ? "text-blue-600"
                : "text-gray-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 ${
              currentScreen === "transactions" && !selectedCategoryId
                ? "text-blue-600"
                : "text-gray-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 ${
              currentScreen === "categories" && !selectedCategoryId
                ? "text-blue-600"
                : "text-gray-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 ${
              currentScreen === "accounts" && !selectedCategoryId
                ? "text-blue-600"
                : "text-gray-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 ${
              currentScreen === "budget" && !selectedCategoryId
                ? "text-blue-600"
                : "text-gray-400"
            }`}
          >
