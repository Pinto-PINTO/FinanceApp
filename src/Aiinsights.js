...
// ─────────────────────────────────────────────
// Calls Firebase Cloud Function via plain fetch.
// Using onRequest (not onCall) so CORS headers
// are set manually and work from any origin.
// ─────────────────────────────────────────────
console.log("KEY:", process.env.REACT_APP_ANTHROPIC_API_KEY);
const FUNCTION_URL =
  "https://us-central1-financetrackerapp-75842.cloudfunctions.net/askClaude";
...
function buildContext({ transactions, categories, accounts, stats }) {
  ...
  const topCats = [...stats.categorySpending]
    ...
    .map(
      (c) =>
        `• ${c.icon} ${c.name}: $${c.spent.toFixed(0)}/$${c.budget.toFixed(0)} budget (${c.percentage.toFixed(0)}%)`,
    )
    .join("\n");
  ...
  const recent = transactions
    ...
    .map((t) => {
      ...
      return `${t.date} | ${t.type === "income" ? "INCOME" : "EXPENSE"} | $${t.amount.toFixed(2)} | ${
...
Income: $${stats.totalIncome.toFixed(2)} | Expenses: $${stats.totalExpenses.toFixed(2)} | Net: $${(
    stats.totalIncome - stats.totalExpenses
  ).toFixed(2)} | Savings Rate: ${rate}%
Accounts: ${accounts.length} | Categories: ${
...
// ─────────────────────────────────────────────
// Single analysis card — styled like the rest of the app
// ─────────────────────────────────────────────
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors ${
            loading ? "bg-gray-300 cursor-not-allowed" : mod.btnClass
          }`}
        >
          {loading ? (
            <>
      ...
      {!loading && !result && (
        <p className="text-sm text-gray-400">
          Click <span className="font-medium text-gray-600">Analyze</span> to
          run this against your live financial data.
        </p>
      )}
        ...
        <div className="space-y-4">
          {sections.map((s, i) => (
            <div key={i}>
              {s.key && (
                <p
                  className={`text-xs font-bold uppercase tracking-widest mb-1 ${mod.accent}`}
                >
                  {s.key}
                </p>
              )}
              {s.value && (
                <p
                  className={`text-sm leading-relaxed ${s.key ? "font-medium text-gray-800" : "text-gray-600"}`}
                >
                  {s.value}
                </p>
              )}
...
// ─────────────────────────────────────────────
// Chat advisor — styled to match app
// ─────────────────────────────────────────────
function ChatAdvisor({ financialContext }) {
  const [msgs, setMsgs] = useState([
    {
      role: "ai",
      text: "Hi! I'm your AI financial advisor. I already have your complete financial data loaded. Ask me anything — spending habits, budget advice, savings goals, or what to focus on.",
    },
  ]);
...
export default function AIInsights({
  ...
  const runModule = async (mod) => {
    ...
    try {
      const result = await askClaude(mod.system, mod.question(ctx));
      setResults((r) => ({ ...r, [mod.id]: result }));
    } catch (e) {
      setResults((r) => ({
        ...r,
        [mod.id]: "Error connecting. Please try again.",
      }));
    } finally {
  ...
  return (
    <div className="space-y-4">
      {/* ── Header card — matches app's gradient header style ── */}
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-blue-100 text-sm">
              Claude analyzes your {transactions.length} transactions across{" "}
              {accounts.length} accounts to surface insights you'd never catch
              manually.
            </p>
          </div>
          {hasData && (
            <button
              onClick={runAll}
              disabled={runningAll}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold bg-white transition-all ${
                runningAll
            ...
            {[
              {
                ...
                color: net >= 0 ? "bg-green-400" : "bg-red-400",
              },
              {
                label: "Savings Rate",
                val: `${savingsRate}%`,
              ...
              {
                label: "Over Budget",
                val: stats.categorySpending.filter((c) => c.percentage > 100)
                  .length,
                sub: "categories",
                color: "bg-yellow-400",
              },
              ...
              <div
                key={s.label}
                className="bg-white bg-opacity-15 rounded-lg px-4 py-2.5 min-w-[90px]"
              >
      ...
      {!hasData && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-sm text-gray-500">
            Add some transactions and accounts first, then come back for AI
            analysis.
          </p>
        </div>
      )}
        ...
        <>
          {MODULES.map((mod) => (
            <AnalysisCard
              key={mod.id}
              mod={mod}
              result={results[mod.id]}
              loading={loading[mod.id] || false}
              onRun={() => runModule(mod)}
            />
          ))}
